...
#include "src/common/Light.hpp"
#include "src/node/primitive/Mesh.hpp"
#include "src/node/primitive/FractalMountain.hpp"
#include "src/node/GeometryNode.hpp"
#include "src/node/bumping/Bumping.hpp"
#include "src/node/texture/Texture.hpp"
#include "src/node/tree/LSystemTree.hpp"
#include "src/node/JointNode.hpp"
#include "src/node/csg/CsgNode.hpp"
#include "src/node/primitive/Primitive.hpp"
#include "src/node/material/Material.hpp"
#include "src/node/material/PhongMaterial.hpp"
...
int gr_union_cmd(lua_State* L)
{
  ...
  luaL_argcheck(L, leftdata != 0, 2, "Node expected");
  GeometryNode* left = dynamic_cast<GeometryNode*>(leftdata->node);

  gr_node_ud* rightdata = (gr_node_ud*)luaL_checkudata(L, 3, "gr.node");
  luaL_argcheck(L, rightdata != 0, 3, "Node expected");
  GeometryNode* right = dynamic_cast<GeometryNode*>(rightdata->node);

  data->node = new CsgNode(name, left, right, CsgOperation::CsgUnion);
...
int gr_diff_cmd(lua_State* L)
{
  ...
  if (!left || !right) {
    std::cerr << "Expected a geometry node" << std::endl;
  }

  data->node = new CsgNode(name, left, right, CsgOperation::CsgDiff);
...
int gr_intersect_cmd(lua_State* L)
{
  ...
  data->node = new CsgNode(name, left, right, CsgOperation::CsgIntersect);
...
// Create a fractal mountain node
extern "C"
int gr_fractal_mountain_cmd(lua_State* L)
{
	GRLUA_DEBUG_CALL;
    ...
    double dimensions [3];
    get_tuple(L, 2, dimensions, 3);

    double roughness = luaL_checknumber(L, 3);
    uint depth = luaL_checknumber(L, 4);
	...
	data->node = new GeometryNode(name, mountain);
...
// Create an LSystemTree
extern "C"
int gr_l_system_tree_cmd(lua_State* L)
{
	GRLUA_DEBUG_CALL;
	...
	const char* name = luaL_checkstring(L, 1);
	const char* grammar_fname = luaL_checkstring(L, 2);

    uint depth = luaL_checknumber(L, 3);

    // finally get the primitive that we want from another geometry node
    // note: we will edit this other geometry node's primitive
    gr_node_ud* childdata = (gr_node_ud*)luaL_checkudata(L, 4, "gr.node");
    luaL_argcheck(L, childdata != 0, 4, "Node expected");
    GeometryNode* steal_trunk_prim = dynamic_cast<GeometryNode *>(childdata->node);

    // finally get the primitive that we want from another geometry node
    // note: we will edit this other geometry node's primitive
    gr_node_ud* childdata_leaf = (gr_node_ud*)luaL_checkudata(L, 5, "gr.node");
    luaL_argcheck(L, childdata_leaf != 0, 5, "Node expected");
    GeometryNode* steal_leaf_prim = dynamic_cast<GeometryNode *>(childdata_leaf->node);

    data->node = new LSystemTree(
        name, grammar_fname, depth,
        steal_trunk_prim,
        steal_leaf_prim
    );
...
// Adds a texture to a certain mesh
extern "C"
int gr_add_texture_cmd(lua_State* L)
{
    GRLUA_DEBUG_CALL;
    ...
    // try and get a geometry node
    GeometryNode *self = dynamic_cast<GeometryNode*> (selfdata->node);
    luaL_argcheck(L, self != 0, 1, "Geometry node expected");

    // get the string name of the file
    const char *texture_fname = luaL_checkstring(L, 2);
    std::string file_name (texture_fname);

    // Use a dictionary structure to make sure every mesh is loaded
    // at most once.
    auto i = texture_map.find(file_name);
...
// Adds a bumping to a certain mesh
extern "C"
int gr_mesh_add_bumps_cmd(lua_State* L)
{
    GRLUA_DEBUG_CALL;
    ...
    Bumping *bumping = nullptr;

    double uv_coord [2], dist_scale[2];
    get_tuple(L, 2, uv_coord, 2);

    double num_period = luaL_checknumber(L, 3);
    // as this number increases, the bumps get more and more spread out
    double period = luaL_checknumber(L, 4);

    // create a new bumping based off of the information given
    // get:
    // 1. (u, v) coordinates of wave
    // 2. number of periods
    // 3. Period of wave

    bumping = new Bumping(
        glm::vec2(uv_coord[0], uv_coord[1]),
        num_period,
        period
    );

    self->setBumping(bumping);
    return 0;
}
...
static const luaL_Reg grlib_functions[] = {
  ...
  // command that gets reflective material
  ...
  {"mesh", gr_mesh_cmd},
  // a new command to generates a fractal mountain
  {"fractal_mountain", gr_fractal_mountain_cmd},
  // a new command to generate an l-system tree
  {"tree", gr_l_system_tree_cmd},
  {"light", gr_light_cmd},
...
static const luaL_Reg grlib_node_methods[] = {
  ...
  {"add_texture", gr_add_texture_cmd},
  {"mesh_add_bumps", gr_mesh_add_bumps_cmd},
